/* v1.5.3.19 — grade settings cleanup, compatible with current modalHtml architecture.
   Scope ONLY:
   - Settings > 학년 관리: show grade names without student counts.
   - Remove the obsolete modalView hook that caused a runtime ReferenceError.
...
(function(){
  ...
  modalHtml = function(){
    if(ui.modal?.type !== 'gradeManager') return priorModalHtml_v15312();
    const gs = grades();
