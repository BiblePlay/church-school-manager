/* v1.5.3.9 — safe attendance-rate sorting on Records screen only.
   Restores the pre-managed-scope behavior everywhere else.
   Does NOT modify talent filters, attendance checklist ordering, settings, student list, or team logic.
*/
(function(){
  'use strict';
  ...
  // Rebuild ONLY the existing records screen from the known-stable v1.4.2.2 behavior,
  // with one added sort control for attendance rate.
  recordsView = function(){
    ...
    // Old managed-scope patches could leave values such as '내 담당' in memory/settings.
    // Records screen only accepts 전체 or a real grade; invalid values must never blank the list.
    if(scope!=='전체' && !availableGrades.includes(scope)){
      scope='전체';
      ui.analyticsScope='전체';
    }
    const baseList = scope==='전체' ? active() : active().filter(s=>s.grade===scope);
    ...
    const scopeButtons = ['전체',...availableGrades];
